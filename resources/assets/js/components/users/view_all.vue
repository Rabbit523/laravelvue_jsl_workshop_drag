...
import axios from 'axios';
	...
	export default {
    	...
    	methods: {
    		...
    		editThis(pageid){
      			...
      			this.$router.push('/getusers/'+pageid+'/edit');

      		},
      		
      		createNew(){

  				this.$router.push('/getusers/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/getusers/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/getusers/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.name.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['name'].localeCompare(b['name']));
    ...
    //     	} else if(this.selected == 'emailAddress'){
