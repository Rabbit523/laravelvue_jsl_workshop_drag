<template>
  ...
  <div>
      ...
      <c-form layout="horizontal" span="220" @submit.prevent="handleSubmit">
        ...
        <c-form-field label="Certificate Name">
          <c-form-input 
              v-validate="'required|min:3'" 
              v-model="form.certificateName" 
              :status="checkErrors('certificate name')" 
              name="certificate name" 
              value="" 
              :disabled="isDisabled"
              type="text">
          </c-form-input>
          <span class="form-help u-color-danger" v-if="!form.errors.has('certificate name')">{{ errors.first('certificate name') }}</span>
          <span class="form-help u-color-danger" v-text="form.errors.get('certificateName')" v-if="form.errors.has('certificateName')"></span>
        </c-form-field>

        <c-form-field label="">
          <c-form-switch label="Expirable" v-model="form.expirable" />
        </c-form-field>
