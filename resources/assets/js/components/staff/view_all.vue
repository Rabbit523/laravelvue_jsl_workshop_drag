	...
	export default {
		data () {
			...
			return {
				...
				toast: '',
				staff: [],
				// headings: ['name', 'employeeCode', 'cellPhone', 'status', 'action'],
				headings: [ 
						{ label: 'Name', name: 'staffName' }, 
						{ label: 'Employee Code', name: 'employeeCode' }, 
						{ label: 'Fahter\'s Name', name: 'fatherName' }, 
						{ label: 'Mobile No', name: 'cellPhone' }, 
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/staff/'+pageid+'/edit');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/staff/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/staff/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.staffName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var cellPhone = !v.selected && v.cellPhone.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var fatherName = !v.selected && v.fatherName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var employeeCode = !v.selected && v.employeeCode.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var sts = !v.selected && v.status.toLowerCase().indexOf(searchText.toLowerCase()) > -1;

		          var finalResult =  name + cellPhone + fatherName + sts + employeeCode;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['staffName'].localeCompare(b['staffName']));
