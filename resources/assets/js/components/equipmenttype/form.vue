<template>
  ...
  <div>
      ...
      <c-form layout="horizontal" span="220" @submit.prevent="handleSubmit">
        ...
        <c-form-field label="Equipment Type">
          <c-form-input 
              v-validate="'required|min:3'" 
              v-model="form.equipmentType" 
              :status="checkErrors('equipment type')" 
              name="equipment type" 
              value="" 
              :disabled="isDisabled"
              type="text">
          </c-form-input>
          <span class="form-help u-color-danger" v-if="!form.errors.has('equipmentType')">{{ errors.first('equipment type') }}</span>
          <span class="form-help u-color-danger" v-text="form.errors.get('equipmentType')" v-if="form.errors.has('equipmentType')"></span>
        </c-form-field>
  ...
  export default {
    ...
    data() {
      return {
        ...
        pageTitle: 'Equipment Type > Add New',
        api: '',
        controller: '/equipmenttype/',
        controllerName: 'Equipment Type',
        basePost: '',
        postUrl: '',
    ...
    methods: {
        ...
        showToast(){
          ...
          // Show success toast
          this.$toast.succeed(this.form.content = this.controllerName + ' Type Data Saved Successfully', this.form.duration = 2000);
