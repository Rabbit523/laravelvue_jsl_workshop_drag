<template>
  ...
  <div>
      ...
      <c-form layout="horizontal" span="220" @submit.prevent="handleSubmit">
        ...
        <c-form-field label="City Name">
          <c-form-input 
              v-validate="'required|min:3'" 
              v-model="form.cityName" 
              :status="checkErrors('city name')" 
              name="city name" 
              value="" 
              :disabled="isDisabled"
              type="text">
          </c-form-input>
          <span class="form-help u-color-danger" v-if="!form.errors.has('cityName')">{{ errors.first('city name') }}</span>
          <span class="form-help u-color-danger" v-text="form.errors.get('cityName')" v-if="form.errors.has('cityName')"></span>
        </c-form-field>
