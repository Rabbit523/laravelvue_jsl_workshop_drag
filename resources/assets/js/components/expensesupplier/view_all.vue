<template>
	<div>
		...
		<c-panel title="Expense Supplier">
		  ...
		  <c-col xs="24" sm="24" md="24" lg="24" xl="24" >
		  	...
		  	<basictable>
		  		...
		  		<tr slot="data" v-for="(item, index) in sortedItems">
		  			...
		  			<td>{{ item.status }}</td>
		  			<td><c-button-group>
						  <c-button :disabled="isDiabled" @click="editThis(item.id)" type="primary"><i class="icon-pencil"></i></c-button>
	...
	export default {
		data () {
			...
			return {
				...
				city: [],
				headings: [ 
						{ label: 'Name', name: 'supplierName' }, 
						{ label: 'Account Number', name: 'accountNumber' }, 
						{ label: 'Phone', name: 'phoneNumber' }, 
						{ label: 'Contact', name: 'contactPerson' }, 
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/expensesupplier/'+pageid+'/edit');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/expensesupplier/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/expensesupplier/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.supplierName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var accountNumber = !v.selected && v.accountNumber.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var contactPerson = !v.selected && v.contactPerson.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var phone1 = !v.selected && v.phone1.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var sts = !v.selected && v.status.toLowerCase().indexOf(searchText.toLowerCase()) > -1;

		          var finalResult =  name + accountNumber + contactPerson + phone1 + sts;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['supplierName'].localeCompare(b['supplierName']));
