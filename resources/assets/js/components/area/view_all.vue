	...
	export default {
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/area/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/area/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/area/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/area/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.areaName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['areaName'].localeCompare(b['areaName']));
