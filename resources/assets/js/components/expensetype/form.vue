<template>
  ...
  <div>
      ...
      <c-form layout="horizontal" span="220" @submit.prevent="handleSubmit">
        ...
        <c-form-field label="Expense Type">
          <c-form-input 
              v-validate="'required|min:3'" 
              v-model="form.expenseType" 
              :status="checkErrors('expense type')" 
              name="expense type" 
              value="" 
              :disabled="isDisabled"
              type="text">
          </c-form-input>
          <span class="form-help u-color-danger" v-if="!form.errors.has('expenseType')">{{ errors.first('expense type') }}</span>
          <span class="form-help u-color-danger" v-text="form.errors.get('expenseType')" v-if="form.errors.has('expenseType')"></span>
        </c-form-field>
  ...
  export default {
    ...
    data() {
      return {
        ...
        pageTitle: 'Expense Type > Add New',
        api: '',
        controller: '/expensetype/',
        controllerName: 'Expense Type',
        basePost: '',
        postUrl: '',
    ...
    methods: {
        ...
        showToast(){
          ...
          // Show success toast
          this.$toast.succeed(this.form.content = this.controllerName + ' Type Saved Successfully', this.form.duration = 2000);
