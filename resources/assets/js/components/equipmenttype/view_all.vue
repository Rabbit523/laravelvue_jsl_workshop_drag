	...
	export default {
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/equipmenttype/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/equipmenttype/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/equipmenttype/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/equipmenttype/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.equipmentType.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['equipmentType'].localeCompare(b['equipmentType']));
