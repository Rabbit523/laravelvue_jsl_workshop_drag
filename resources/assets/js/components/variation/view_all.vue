	...
	export default {
		data () {
			...
			return {
				...
				headings: [ 
						{ label: 'Variation Name', name: 'variationName' }, 
    	...
    	created() {
    		...
    		axios
        		.get('/variation')
        			.then(response => {
            		// this.variation = response.data.data;
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/variation/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/variation/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/variation/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/variation/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.variationName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['variationName'].localeCompare(b['variationName']));
