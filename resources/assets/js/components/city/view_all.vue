	...
	export default {
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/city/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/city/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/city/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/city/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.cityName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['cityName'].localeCompare(b['cityName']));
