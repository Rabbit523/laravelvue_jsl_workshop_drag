	...
	export default {
		data () {
			...
			return {
				...
				city: [],
				headings: [ 
						{ label: 'Name', name: 'certificateName' }, 
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/certificate/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/certificate/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/certificate/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/certificate/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.certificateName.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		      ...
		      this.total = Math.round((this.filteredItems.length / this.pagination.itemPerPage), 0);
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            var newSort = result.sort((a, b) => ascDesc * a['certificateName'].localeCompare(b['certificateName']));
