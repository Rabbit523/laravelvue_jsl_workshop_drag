		...
		<c-row>
		  <c-col xs="24" sm="24" md="24" lg="24" xl="24" >
		  	...
		  	<basictable>
		  			...
		  			<th v-for="heading in headings" slot="heading" @click="invertSort(), selected = heading" >
		  				{{ heading }}
		  				<i v-if="currentSortDir == 1" :class="{ 'is-down':selected == heading }" class="i-sort"></i>
		  				<i v-if="currentSortDir == 0" :class="{ 'is-up':selected == heading }" class="i-sort"></i>
		  			</th>
	...
	export default {
    	...
    	methods: {
			...
			editThis(pageid){
  				...
  				this.$router.push('/expensetype/'+pageid+'/edit');

  			},

  			createNew(){

  				this.$router.push('/expensetype/addnew');
          ...
          ActiveorInActive: function (id, index){
          	...
          	this.handleSubmit('status', 'post', '/expensetype/status/', this.form.statusid, this.form.status, i, index);
          ...
          killIt: function (id, index){
          	...
          	this.handleSubmit('kill', 'get', '/expensetype/destroy/', this.form.statusid, this.form.status, i, index, p);
          ...
          searchInTheList(searchText, currentPage, statusfilter){
		        ...
		        this.filteredItems = _.filter(this.items, function(v, k){
		          ...
		          var name = !v.selected && v.expenseType.toLowerCase().indexOf(searchText.toLowerCase()) > -1;
		          var sts = !v.selected && v.status.toLowerCase().indexOf(searchText.toLowerCase()) > -1;

		          var finalResult =  name +  sts;
    	...
    	computed:{
		  ...
		  sortedItems() {
            ...
            if(this.selected == 'name' || this.selected == undefined){

            	return result.sort((a, b) => ascDesc * a['expenseType'].localeCompare(b['expenseType']));

        	} else if(this.selected == 'email'){
        		
        		return result.sort((a, b) => ascDesc * a['emailAddress'].localeCompare(b['emailAddress']));

        	} else if (this.selected == 'status'){

				return result.sort((a, b) => ascDesc * a['status'].localeCompare(b['status']));        		
        	}
